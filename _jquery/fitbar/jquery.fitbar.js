...
;(function($){
  ...
  $.extend($.fitbar.prototype, {
    ...
    adjustPosition : function(){
      ...
      //改造：リサイズ時に固定エリアの再設定を行う。リサイズイベントが終わって２秒後に実行する。
      if(isFixed){
        function ft_startTimer(status){
          if(status == "init"){
            clearTimeout(_fitbarResizeCmdTimer);
            _ft_nowCnt = 2;
          }
          // _fitbarResizeCmdTimer = setTimeout("ft_timerCountUp()",1000);
          _fitbarResizeCmdTimer = setTimeout(function(){ft_timerCountUp();},1000);
        }
        function ft_timerCountUp(){
          _ft_nowCnt--;
          if(_ft_nowCnt > 0){
            ft_startTimer("-");
          }else{
